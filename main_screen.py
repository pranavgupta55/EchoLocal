...
for folder in folders:
    for songPath in os.listdir(folder):
        if songPath.startswith("."):
            continue
        if songPath.lower().endswith(".mp3"):
            songs.append(os.path.join(folder, songPath))

songs.sort()

def getSongImage(mp3Path,imageSize):
    try:
        audio = ID3(mp3Path)
        for tag in audio.values():
            if isinstance(tag,APIC):
                imageData = tag.data
                pilImage = Image.open(io.BytesIO(imageData)).convert("RGBA")
                mode = pilImage.mode
                size = pilImage.size
                data = pilImage.tobytes()
                surface = pygame.image.fromstring(data,size,mode)
                surface = pygame.transform.smoothscale(surface,(imageSize,imageSize))
                return surface
    except Exception as e:
        print(f"Could not load artwork for {mp3Path}")
        print(e)

    fallback = pygame.Surface((imageSize,imageSize))
    fallback.fill((0,0,0))
    return fallback
...
for songPath in songs:
    songImages.append(getSongImage(songPath,imageSize))

print(f"Loaded {len(songImages)} images")
...
class FireCols:
    cols = [[255, 242, 157], [255, 212, 124], [255, 169, 70], [254, 128, 3], [230, 106, 37], [179, 46, 19],
            [126, 19, 6], [70, 2, 14]]
    colsReversed = [[70, 2, 14], [126, 19, 6], [179, 46, 19], [230, 106, 37], [254, 128, 3], [255, 169, 70],
                    [255, 212, 124], [255, 242, 157]]
...
while running:
    ...
    if toggle:
        ...
        for i, label in enumerate(items.keys()):
            ...
            drawText(screenUI, Endesga.debug_red, montserratRegularAdaptive, 5,
                     screen_height - (30 + 25 * i) / (scaleDownFactor ** (1 / 1.8)), string, Endesga.black,
                     int(3 / scaleDownFactor) + int(3 / scaleDownFactor) < 1, antiAliasing=False)
